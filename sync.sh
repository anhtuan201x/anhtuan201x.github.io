...
if [ "$total_packages" -eq 0 ]; then total_packages=45; fi
...
body { margin: 0; padding: 20px; min-height: 100vh; display: flex; align-items: center; justify-content: center; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; background: linear-gradient(135deg,var(--bg1),var(--bg2),var(--bg3)); color: var(--text); overflow-x: hidden; }
#galaxyCanvas { position: fixed; inset: 0; width: 100%; height: 100%; z-index: 0; pointer-events: none; }
.container { position: relative; z-index: 2; width: 100%; max-width: 440px; padding: 35px 25px; text-align: center; border-radius: 24px; background: var(--card); backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); box-shadow: 0 20px 50px rgba(0,0,0,.5); }
.logo { width: 90px; height: 90px; object-fit: cover; border-radius: 22px; margin-bottom: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); }
h1 { margin: 0 0 10px; font-size: 30px; font-weight: 800; background: linear-gradient(45deg,#ff416c,#ff4b2b); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; }
.description { font-size: 15px; font-weight: 600; margin-bottom: 20px; }
.story { color: var(--sub); font-size: 13.5px; line-height: 1.6; text-align: left; padding: 15px 0; border-top: 1px solid rgba(128,128,128,.25); border-bottom: 1px solid rgba(128,128,128,.25); }
.stats { display: grid; grid-template-columns: repeat(3,1fr); gap: 10px; margin: 22px 0; }
.stat { padding: 12px 5px; border-radius: 12px; background: rgba(128,128,128,.1); }
.stat-number { font-size: 16px; font-weight: 800; color: #ff4b2b; }
.stat-label { margin-top: 3px; font-size: 11px; color: #888; }
.btn { display: block; width: 100%; padding: 14px; margin-bottom: 10px; border-radius: 25px; color: white; text-decoration: none; font-size: 15px; font-weight: 700; transition: .2s; text-align: center; }
.btn:hover { transform: scale(.97); }
.cydia { background: linear-gradient(135deg,#4cd964,#28c840); }
.cert-btn { background: linear-gradient(135deg,#ff9500,#ffcc00); box-shadow: 0 4px 10px rgba(255,149,0,0.3); }
.music-btn { position: absolute; right: 15px; top: 15px; border: 0; padding: 7px 12px; border-radius: 15px; cursor: pointer; background: rgba(128,128,128,.15); color: var(--text); font-weight: bold; }
...
<a href="cydia://url/https://saurik.com" class="btn cydia">➕ Add to Cydia</a>
<!-- ĐÃ ĐỔI LINK NÚT BẤM NHẢY THẲNG TRỰC TIẾP SANG LITTEN THEO ĐÚNG Ý TUẤN -->
...
# ==========================================
# 4. XUẤT BẢN FILE RELEASE KHÓA CỨNG IPHONEOS-ARM CHUẨN ĐÉT
# ==========================================
cat <<EOF > Release
